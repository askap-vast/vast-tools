...
This tutorial shows you how to obtain one: http://99rabbits.com/get-dropbox-access-token/
...
As seen above the script takes in one positional argument which is the Dropbox configuration file. Create a text file in the following format and entering the respective values:
```
[dropbox]
...
```
There is no need to put quotes around the strings. A template can be generated by using `get_vast_pilot_dbx.py --write-config`.
